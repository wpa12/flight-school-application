        ...
        <section class="border-b border-slate-200/80 bg-linear-to-b from-sky-50 via-white to-white py-8 dark:border-slate-800 dark:from-slate-900 dark:via-slate-950 dark:to-slate-950 sm:py-10">
            <div class="mx-auto max-w-2xl px-4 sm:px-6 lg:px-8">
                <nav class="text-sm text-slate-600 dark:text-slate-400" aria-label="Breadcrumb">
                    ...
                    <span class="text-slate-900 dark:text-white">Add Aircraft</span>
                </nav>
                <h1 class="mt-3 text-2xl font-semibold tracking-tight text-slate-900 dark:text-white">Add aircraft</h1>
            </div>
        </section>

        <section class="bg-white py-10 dark:bg-slate-950 sm:py-12">
            <div class="mx-auto max-w-2xl px-4 sm:px-6 lg:px-8">
                <form method="post" action="{{ route('dashboard.aircraft.store') }}" class="space-y-5 rounded-2xl border border-slate-200 bg-slate-50/80 p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900/60 sm:p-8">
                    @csrf
                    @if ($errors->any())
                    ...
                    <div class="grid gap-5 sm:grid-cols-2">
                        <div>
                            ...
                            <input id="make" name="make" type="text" value="{{ old('make') }}" required class="mt-1 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 shadow-sm focus:border-sky-500 focus:outline-none focus:ring-1 focus:ring-sky-500 dark:border-slate-600 dark:bg-slate-900 dark:text-white" />
                        </div>
                        <div>
                            <label for="model" class="block text-sm font-medium text-slate-700 dark:text-slate-300">Model</label>
                            <input id="model" name="model" type="text" value="{{ old('model') }}" required class="mt-1 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 shadow-sm focus:border-sky-500 focus:outline-none focus:ring-1 focus:ring-sky-500 dark:border-slate-600 dark:bg-slate-900 dark:text-white" />
                        </div>
                    </div>
                    <div>
                        <label for="registration" class="block text-sm font-medium text-slate-700 dark:text-slate-300">Registration</label>
                        <input id="registration" name="registration" type="text" value="{{ old('registration') }}" required class="mt-1 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 font-mono text-sm text-slate-900 shadow-sm focus:border-sky-500 focus:outline-none focus:ring-1 focus:ring-sky-500 dark:border-slate-600 dark:bg-slate-900 dark:text-white" />
                    </div>
                    <div>
                    ...
                    <div class="grid gap-5 sm:grid-cols-2">
                        <div>
                            ...
                            <input id="rental_price_per_hour" name="rental_price_per_hour" type="number" step="0.01" min="0" max="1000" value="{{ old('rental_price_per_hour') }}" required class="mt-1 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 shadow-sm focus:border-sky-500 focus:outline-none focus:ring-1 focus:ring-sky-500 dark:border-slate-600 dark:bg-slate-900 dark:text-white" />
                        </div>
                        <div>
                            <label for="current_hours" class="block text-sm font-medium text-slate-700 dark:text-slate-300">Current hours</label>
                            <input id="current_hours" name="current_hours" type="number" min="0" max="20000" value="{{ old('current_hours', 0) }}" required class="mt-1 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 shadow-sm focus:border-sky-500 focus:outline-none focus:ring-1 focus:ring-sky-500 dark:border-slate-600 dark:bg-slate-900 dark:text-white" />
                        </div>
                    </div>
                    <div>
                        <label for="image_url" class="block text-sm font-medium text-slate-700 dark:text-slate-300">Image URL</label>
                        <input id="image_url" name="image_url" type="url" value="{{ old('image_url') }}" class="mt-1 w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 shadow-sm focus:border-sky-500 focus:outline-none focus:ring-1 focus:ring-sky-500 dark:border-slate-600 dark:bg-slate-900 dark:text-white" placeholder="https://…" />
                    </div>
                    <div class="flex items-center gap-2">
                        <input type="hidden" name="in_service" value="0" />
                        <input id="in_service" name="in_service" type="checkbox" value="1" class="size-4 rounded border-slate-300 text-sky-600 focus:ring-sky-500 dark:border-slate-600 dark:bg-slate-900" @checked(old('in_service', '1') === '1') />
                    ...
                    <div class="flex flex-wrap gap-3 pt-2">
                        ...
                        <a href="{{ route('aircraft') }}" class="inline-flex items-center justify-center rounded-lg border border-slate-300 bg-white px-4 py-2.5 text-sm font-semibold text-slate-700 shadow-sm transition hover:bg-slate-50 dark:border-slate-600 dark:bg-slate-900 dark:text-slate-200 dark:hover:bg-slate-800">
                            Cancel
                        </a>
                    </div>
                </form>
